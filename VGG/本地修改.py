...
def train_data_process():
    ...
    # plt.show()
    plt.savefig('/root/2024 NN Curriculum Design/train_fig_output/one batch figure.png')
...
def train_model(model, traindataloader, train_rate, criterion, device, optimizer, num_epochs=25):
    ...
    best_model_wts = copy.deepcopy(model.state_dict())  # 复制当前VGG模型的参数
    # 初始化参数
    best_acc = 0.0  # 最高准确度
    ...
    plt.legend()
    # plt.show()
    plt.savefig('/root/2024 NN Curriculum Design/data_analysis_output/data.png')
